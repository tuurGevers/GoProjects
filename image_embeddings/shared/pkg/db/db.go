...
func FetchCollection() (*http.Response, error) {
	...
	payload := strings.NewReader("{\"collectionName\":\"image_testsv2\",\"id\":[1,2,3]}")
...
func SearchData(vectors []float64) (*http.Response, error) {
	...
	payloadString := fmt.Sprintf("{\"collectionName\":\"image_testsv2\",\"vector\":[%v]}", strings.Trim(strings.Join(strings.Fields(fmt.Sprint(vectors)), ","), "[]"))
...
func InsertData(vectors []float64) (*http.Response, error) {
	...
	payloadString := fmt.Sprintf("{\"collectionName\":\"image_testsv2\",\"data\":[{\"vector\":[%v]}]}", strings.Trim(strings.Join(strings.Fields(fmt.Sprint(vectors)), ","), "[]"))
	payload := strings.NewReader(payloadString)
